...
class DereTablePeriod(models.Model):
    ...
    def _event_can_be_generated(self, event_type):
        ...
        return not latest or latest.state in ("draft", "generated", "rejected")

    @api.depends("event_ids.state", "event_ids.event_type", "batch_ids.state")
    ...
    def tables_accepted(self):
        self.ensure_one()
        return all(
            event and event.state == "accepted"
            for event in (
                self._latest_event(EVENT_D1001),
                self._latest_event(EVENT_D1011),
            )
        )
