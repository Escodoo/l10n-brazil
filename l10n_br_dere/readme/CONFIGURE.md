...
7. If the taxpayer must send D-1106, enable **Subject to D-1106**, mark the
   ...
   Accounts whose `codTrib` is in the official D-1106 list also require the
   event. Keep one asset per account so **Generate D-1106** can fill amounts
   from posted journal items. Optionally set **DeRE reserve income account**
...
are reused by every monthly declaration in force.
...
- referential code, nature and **required** `codTrib` (empty referential
  values inherit from the prefix group)
