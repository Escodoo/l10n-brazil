...
class FiscalDocumentLineMixinMethods(models.AbstractModel):
    ...
    def _build_null_mask_dict(self) -> dict:
        ...
        mask_dict = {
            f[0]: False
            for f in filter(
                lambda f: f[1].compute == "_compute_tax_amounts",
    ...
    def write(self, vals):
        ...
        tax_fields_in_vals = [fld for fld in vals if fld in ALL_TAX_ID_FIELDS]
    ...
    def _onchange_fiscal_taxes(self):
        self._update_fiscal_tax_ids()
        # pelo que notei o inverse é chamado depois de toda a recomputação.
        # e a alteração aqui não chama a computação novamente.
    ...
    def _add_fields_to_amount(self):
        ...
        if (
            ...
            and self.fiscal_operation_id.fiscal_operation_type == "in"
        ):
