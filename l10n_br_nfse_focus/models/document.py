...
class Document(models.Model):
    ...
    def _document_export(self, pretty_print=True):
        ...
        for record in self.filtered(filter_processador_edoc_nfse).filtered(
            ...
            # Ensure document_date is set before creating event
            if not record.document_date or not isinstance(
                record.document_date, (date, datetime)
            ):
                # Use authorization_date if available, otherwise use current date
                if record.authorization_date:
                    record.document_date = record.authorization_date.date()
                else:
                    record.document_date = fields.Date.today()
